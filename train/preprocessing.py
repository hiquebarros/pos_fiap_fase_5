...
def get_feature_target(df: pd.DataFrame, include_bmi: bool = True) -> tuple[pd.DataFrame, pd.Series]:
...
def build_preprocessor(include_bmi: bool = True) -> ColumnTransformer:
    num_cols = numeric_features(include_bmi)
    return ColumnTransformer(
        transformers=[
            ...
            (
                "ord",
                OrdinalEncoder(categories=[CAEC_ORDER, CALC_ORDER]),
                ORDINAL_FEATURES,
            ),
        ]
    )
...
def build_model_pipeline(classifier, include_bmi: bool = True) -> Pipeline:
    return Pipeline(
        steps=[
            ("preprocessor", build_preprocessor(include_bmi)),
